# Fork of isaacgym/python/examples/joint_monkey.py for WSLg: configurable num_envs (default 1).
# Upstream hard-codes 36 envs, which often crashes the viewer on WSL even with CPU + cartpole.
...
_ASSET_ROOT = os.path.join(_REPO_ROOT, "isaacgym", "assets")
...
args = gymutil.parse_arguments(
    description="joint_monkey_wsl: fork with --num_envs for WSLg stability",
    custom_parameters=[
        {"name": "--asset_id", "type": int, "default": 0, "help": "Asset id (0 - %d)" % (len(asset_descriptors) - 1)},
        {"name": "--speed_scale", "type": float, "default": 1.0, "help": "Animation speed scale"},
        {"name": "--show_axis", "action": "store_true", "help": "Visualize DOF axis"},
        {"name": "--num_envs", "type": int, "default": 1, "help": "Parallel envs (use 1 on WSL; upstream uses 36)"},
        {"name": "--no_sync", "action": "store_true", "help": "Skip sync_frame_time (debug WSL crashes)"},
    ],
)
...
if args.num_envs < 1 or args.num_envs > 256:
    print("*** num_envs must be in 1..256")
    sys.exit(1)

if not os.path.isdir(_ASSET_ROOT):
    print("*** Asset root not found:", _ASSET_ROOT)
    sys.exit(1)
...
print("Loading asset '%s' from '%s'" % (asset_file, _ASSET_ROOT))
asset = gym.load_asset(sim, _ASSET_ROOT, asset_file, asset_options)
...
num_per_row = max(1, int(math.ceil(math.sqrt(num_envs))))
spacing = 2.5
...
print("Creating %d environments (num_per_row=%d)" % (num_envs, num_per_row))
